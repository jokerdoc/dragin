...
class Dragin {
  ...
  // 设置配置
  setOptions({ clone = null, dir = 'hor,ver', limitX = [], limitY = [], start = util.noop, move = util.noop, end = util.noop }) {
    this.clone = clone;
    let temp = dir.split(',');
  ...
  create() {
    ...
    else if (util.isFunction(this.clone)) {
      this.mirror = util.parseHTML(this.clone());
    }
    else {
      this.mirror = util.parseHTML(this.clone);
    }
